...
|-------|-------------|----------|
| UserAccountEvents | Creates/modifies/deletes users | OpenDirectory.framework APIs (home artifacts under `/tmp/edr_telem_userhomes`) |
...
Note: modern hardened macOS versions frequently block kext load/unload operations; API-level attempts are still made for telemetry validation, but success is not expected.

### 8. Code Signing & Trust Activity

| Event | Description | Mechanism |
|-------|-------------|-----------|
| CodeSignTrust | Signature/trust enrichment validation on trusted vs tampered samples + XProtect metadata | Security.framework + process exec + plist reads |

This category covers:
- Code signature verification via Security.framework APIs
- Tampered binary generation from a signed system binary
- Trusted vs tampered execution to generate process telemetry with different trust outcomes
...
| ProcessAccess | Attempts task port access + tracing | task_for_pid(), ptrace() |
...
Profile add/remove telemetry (`ES_EVENT_TYPE_NOTIFY_PROFILE_ADD` and `ES_EVENT_TYPE_NOTIFY_PROFILE_REMOVE`) is a strong macOS signal to add next. It is not currently generated by this script.
...
| `UserAccountEvents` fails with OpenDirectory errors | Verify root execution and local directory node availability (`/Local/Default`) |
...
### Kext Load/Unload Expectations

| Symptom | Cause | Behavior |
|--------|-------|----------|
| Kext load/unload attempts return non-zero and no obvious KEXT notify events | Modern macOS hardening and policy restrictions on kernel extensions | Expected in many environments; generator still performs direct KextManager API calls for telemetry validation. |
